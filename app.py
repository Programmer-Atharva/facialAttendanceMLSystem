    ...
    def fill_attendance():
        ...
        try:
            ...
            connection = pymysql.connect(
                host='localhost', user='root', password='', db='manually_fill_attendance',port=3306)
        ...
        if subb == '':
            ...
            def create_csv():
                ...
                csv_name = 'path-to_project/Attendance/Manually Attendance/'+DB_table_name+'.csv'
            ...
            if sub == '':
                ...
                for col in col_names:
                    if col not in df.columns:
                        raise KeyError(f"required column '{col}' is missing from csv file")                                        
                cam = cv2.VideoCapture(0)
                    ...
                    for (x, y, w, h) in faces:
                        ...
                        if (conf < 70):
                            ...
                            try:
                                ...
                                if len(aa)==0:
                                    raise ValueError(f"No match found for Id : {Id}")
                ...
                try:
                    global cursor
                    connection = pymysql.connect(
                        host='localhost', user='root', password='', db='Face_reco_fill',port=3306)
                    cursor = connection.cursor()
                except Exception as e:
                    print(e)
                sql = "CREATE TABLE " + DB_Table_name + """
    ...
    def log_in():
        ...
        if username == 'username':
            if password == 'password':
                ...
                root.configure(background='grey80')
                cs = 'path-to_project/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
                    for col in reader:
